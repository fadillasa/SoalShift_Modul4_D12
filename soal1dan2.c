		    ...
		    struct fuse_file_info *fi)
{
	...
	if (strstr(dirpath,"Documents")>0 && (strstr(fpath,".pdf")>0 || strstr(fpath,".doc")>0 || strstr(fpath,".txt")>0)){
		// ---- no1 ----		
		...
		if(strstr(fpath,"rahasia")==NULL){
			char c[] = {""};
			sprintf(c,"%s/rahasia",dirpath);
			DIR *dir = opendir(c);
			if (dir == NULL){
				char s[]={""};				
				sprintf(s,"mkdir %s/rahasia",dirpath);
				system(s);
			}
			char mod[] = {"chmod 000 "};
			strcat(mod,dirpath);
